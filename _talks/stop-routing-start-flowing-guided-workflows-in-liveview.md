...
* A UX-first argument for constrained navigation. Fewer choices at each step means less cognitive load — and that principle should shape how we structure the code.
* Error recovery without defensive code. How compile-time macros can apply Erlang's "let it fail" philosophy to LiveView — wrapping callback boundaries so that unexpected failures are caught, logged with full context, and resolved by returning the user to a known-good state.
